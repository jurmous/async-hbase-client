...
/**
 * A Callable<T> that will be retried.  If {@link #call(ResponseHandler)} invocation throws exceptions,
...
public interface AsyncRetryingCallable<T> {
  /**
   * Prepare by setting up any connections to servers, etc., ahead of {@link #call(ResponseHandler)} invocation.
   *
   * @param reload Set this to true if need to requery locations (usually set on second invocation
   *               to {@link #call(ResponseHandler)} )} or whatever
   * @throws IOException e
   */
  void prepare(final boolean reload) throws IOException;

  /**
   * Called when {@link #call(ResponseHandler)} throws an exception and we are going to retry; take action to
   ...
   *
   * @param handler handler for computed result
   */
  public void call(final ResponseHandler<T> handler);
}
